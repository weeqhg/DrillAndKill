...
public class MovementController
{
    ...
    public void HandleMovement(Vector2 moveInput, bool isGrounded)
    {
        ...
        bool onSlope = isGrounded && slopeAngle > 3f;
        bool tooSteep = slopeAngle > _maxSlopeAngle;

        if (tooSteep)
        {
            Vector3 slopeDir = Vector3.ProjectOnPlane(Vector3.down, groundNormal).normalized;

            if (Vector3.Dot(moveDirection, slopeDir) < 0)
            {
    ...
    private float GetTargetSpeed(Vector2 moveInput)
    {
        ...
        float baseSpeed = canSprint ? _player.MoveSpeed * 1.65f : _player.MoveSpeed;
    ...
    {
        _currentBonusSpeed *= Mathf.Pow(1f - _bonusDecayPercent, Time.fixedDeltaTime);
    }
}