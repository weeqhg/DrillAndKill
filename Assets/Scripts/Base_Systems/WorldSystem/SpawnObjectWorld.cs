...
public class SpawnObjectWorld : MonoBehaviour
{
    ...
    [SerializeField] private List<SpawnedObject> decorObjectDatas = new();
    [SerializeField] private List<SpawnedObject> interactiveObjectDatas = new();
    [SerializeField] private List<GameObject> decorSpawnedObjects = new();
    ...
    {
        difficulty = GameManager.Instance.difficultyManager.GetDifficultySnapshot();
        SpawnAllInteractive();
        GameEvents.OnCommandObjectSpawn += SpawnInteractiveHandler;
    }
    ...
    private void TrySpawnObject(SpawnedObject spawn, bool isInteractive)
    {
        ...
        for (int attempt = 0; attempt < 10; attempt++)
        {
            ...
            else SpawnDecorObject(spawn, worldPos);
            return;
        }
    }
    ...
    private bool IsPositionOccupied(Vector3 position, float radius)
    {
        ...
        // decor
        foreach (var obj in decorSpawnedObjects)
        {
            if (obj == null) continue;
    ...
    private void SpawnDecorObject(SpawnedObject spawn, Vector3 position)
    {
        Ray ray = new Ray(position + Vector3.up * 500f, Vector3.down);
    ...
    public void SpawnDecorObjects()
    {
        ClearDecorObject();

        foreach (var spawn in decorObjectDatas)
        {
            for (int i = 0; i < spawn.count; i++)
            {
